...
## Fundo reativo
`atmosphere.js`, canvas único (DPR ≤1.5): campos de papel (deriva + parallax) + retícula editorial latente — microcélulas de tinta (18px, alpha base 0.045) que despertam ao redor do ponteiro com inércia e decaimento (teto alpha 0.17, raio ~165–240px por velocidade). Pausa fora da viewport e com aba oculta; reduced-motion = quadro estático; mobile sem interação.
...
## CTA
`.btn--hero`: bloco de tinta, texto papel, seta em `--ember-dark`; hover = seta desliza + fundo `--ink-soft`, botão parado.
