/* Preloader → hero: uma única animação. Fragmentos montam o V, a brasa
   acende no vértice, tudo se dispersa para cima e o palco da hero entra.
   Só roda quando o head marcou html.intro (primeira visita da sessão,
   motion permitido). Fallback: timeout de segurança remove o overlay. */
...
export function initPreloader() {
  ...
  if (!root.classList.contains('intro') || !pre) {
    // visita recorrente ou reduced motion: nada a fazer, hero já visível
    if (pre) pre.remove();
    return;
  }
  ...
  // y: 0 limpa o offset em px herdado do transform da classe .intro
  gsap.set(lines, { y: 0, yPercent: 110 });
  gsap.set(stages, { autoAlpha: 0, y: 14 });
  // a partir daqui o controle é do GSAP
  ...
  // posições de montagem: dois traços do V
  const cluster = [
    { x: -52, y: -58, r: -18 }, // traço esquerdo "\"
    { x: -37, y: -19, r: -18 },
    { x: -22, y: 20, r: -18 },
    { x: -8, y: 56, r: -18 },
    { x: 50, y: -58, r: 18 },   // traço direito "/"
    { x: 28, y: -4, r: 18 },
    { x: 8, y: 50, r: 18 },
  ];
  ...
  // dispersão: tudo perde peso e sobe
  ...
  // passagem de bastão para a hero
  tl.add(heroIn(false), 1.1);

  // segurança: se algo travar, o site nunca fica bloqueado
  setTimeout(() => {
